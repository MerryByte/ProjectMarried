...
let lastCameraTap = 0;
takePhotoButton.addEventListener("click", event => { const now=Date.now(); if(now-lastCameraTap<350){ event.preventDefault(); return; } lastCameraTap=now; captureInSelectedMode(); });
takePhotoButton.addEventListener("dblclick", event => { event.preventDefault(); event.stopPropagation(); });
...
const shareSection=document.querySelector("#share"), preweddingSection=document.querySelector("#prewedding-gallery"), registrySection=document.querySelector("#registry");
if (shareSection&&preweddingSection&&registrySection) { shareSection.after(preweddingSection); preweddingSection.after(registrySection); }
initializePreweddingGalleryPublic();
async function getConfig() { if (window.WEDDING_CONFIG?.supabaseUrl && window.WEDDING_CONFIG?.anonKey) return window.WEDDING_CONFIG; const response=await fetch("/api/upload-config",{cache:"no-store"}); const config=await response.json(); if(!response.ok) throw new Error(config.error||"Gallery configuration is unavailable."); return config; }
async function initializePreweddingGalleryPublic() { const section=document.querySelector("#prewedding-gallery"), carousel=document.querySelector("#preweddingCarousel"); if(!section||!carousel)return; try { const config=await getConfig(), bucket=config.publicBucket||"wedding-prewedding"; const response=await fetch(`${config.supabaseUrl}/storage/v1/object/list/${bucket}`,{method:"POST",headers:{apikey:config.anonKey,"Content-Type":"application/json"},body:JSON.stringify({prefix:"prewedding",limit:100,sortBy:{column:"name",order:"asc"}})}); const files=await response.json(); if(!response.ok)return; files.filter(file=>/\.(jpe?g|png|webp|gif)$/i.test(file.name||"")).forEach((file,index)=>{const img=document.createElement("img");img.src=`${config.supabaseUrl}/storage/v1/object/public/${bucket}/prewedding/${encodeURIComponent(file.name)}`;img.alt="Our story before the wedding";if(!index)img.className="active";carousel.append(img)}); if(!carousel.children.length)return; section.hidden=false;let current=0;setInterval(()=>{carousel.children[current].classList.remove("active");current=(current+1)%carousel.children.length;carousel.children[current].classList.add("active")},5000)}catch{} }
async function initializePreweddingGallery() { const section=document.querySelector("#prewedding-gallery"), carousel=document.querySelector("#preweddingCarousel"); if(!section||!carousel)return; try { const config=await getConfig(); const response=await fetch(`${config.supabaseUrl}/storage/v1/object/list/${config.bucket}`,{method:"POST",headers:{apikey:config.anonKey,"Content-Type":"application/json"},body:JSON.stringify({prefix:"prewedding",limit:100,sortBy:{column:"name",order:"asc"}})}); const files=await response.json(); if(!response.ok)return; files.filter(file=>/\.(jpe?g|png|webp|gif)$/i.test(file.name||"")).forEach((file,index)=>{const img=document.createElement("img");img.src=`${config.supabaseUrl}/storage/v1/object/public/${config.bucket}/prewedding/${encodeURIComponent(file.name)}`;img.alt="Our story before the wedding";if(!index)img.className="active";carousel.append(img)}); if(!carousel.children.length)return; section.hidden=false;let current=0;setInterval(()=>{carousel.children[current].classList.remove("active");current=(current+1)%carousel.children.length;carousel.children[current].classList.add("active")},5000)}catch{} }
